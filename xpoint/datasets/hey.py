...
import csregnet.utils as utils
...
class ImagePairDataset(Dataset):
    """
    Class to load a sample from a given hdf5 file or folder.
    """
    ...
    def __init__(self, config):
        ...
        if self.config["keypoints_filename"] is not None:
            ...
            memberlist_checklist = self.memberslist.copy()
            #remove the extension from the memberlist_checklist for every element if exists :
            for i in range(len(memberlist_checklist)):
                memberlist_checklist[i] = memberlist_checklist[i].rsplit(".", 1)[0]


            for item in memberlist_checklist:
                if item not in keypoint_members:
                    missing_labels.append(item)

            if len(missing_labels) > 0:
                raise IndexError(
                    "Labels for the following samples not available: {}, len : {}".format(
    ...
    def __getitem__(self, index):
        ...
        if self.config["keypoints_filename"] is not None:
            with h5py.File(
                self.config["keypoints_filename"], "r", swmr=True
            ) as keypoints_file:
                if (
                    "keypoints_optical"
                    in keypoints_file[self.memberslist[index]].keys()
                ):
                    keypoints = [
                        np.array(
                            keypoints_file[self.memberslist[index]]["keypoints_optical"]
                        ),
                        np.array(
                            keypoints_file[self.memberslist[index]]["keypoints_thermal"]
                        ),
                    ]
                else:
                    keypoints = np.array(
        ...
        if self.config["height"] > 0 or self.config["width"] > 0:
            if self.config["height"] > 0:
                h = (
                    self.config["height"] // 32 * 32
                )  # make sure it is divisible by 32(encoder downsampling ratio)
            else:
                h = thermal.shape[0]

            if self.config["width"] > 0:
                w = (
                    self.config["width"] // 32 * 32
                )  # make sure it is divisible by 32(encoder downsampling ratio)
            else:
                w = thermal.shape[1]
            ...
            if keypoints is not None:
                if type(keypoints) is list:
                    ...
                    keypoints[0] = keypoints[0][
                        np.logical_and(
                            np.logical_and(
                                keypoints[0][:, 0] >= 0, keypoints[0][:, 0] < h
                            ),
                            np.logical_and(
                                keypoints[0][:, 1] >= 0, keypoints[0][:, 1] < w
                            ),
                        )
                    ]
                    keypoints[1] = keypoints[1][
                        np.logical_and(
                            np.logical_and(
                                keypoints[1][:, 0] >= 0, keypoints[1][:, 0] < h
                            ),
                            np.logical_and(
                                keypoints[1][:, 1] >= 0, keypoints[1][:, 1] < w
                            ),
                        )
                    ]
                else:
        ...
        if self.config["single_image"]:
            ...
            if self.config["augmentation"]["homographic"]["enable"]:
                image, keypoints, valid_mask = augmentation.homographic_augmentation(
                    image, keypoints, **self.config["augmentation"]["homographic"]
                )
            else:
            ...
            if keypoints is not None:
                ...
                out["keypoints"]["optical"] = torch.from_numpy(
                    keypoints[0].astype(bool)
                )
                out["keypoints"]["thermal"] = torch.from_numpy(
                    keypoints[1].astype(bool)
                )
            ...
            if self.config["augmentation"]["homographic"]["enable"]:
                ...
                if keypoints is not None:
                    ...
                    if type(keypoints) is not list:
                        my_keypoints = [
                            keypoints,
                            keypoints,
                        ]  # added for working in non window cases
                if bool(random.randint(0, 1)):
                    valid_mask_thermal = augmentation.dummy_valid_mask(thermal.shape)

                    keypoints_thermal = my_keypoints[1]
                    optical, keypoints_optical, valid_mask_optical, H = (
                        augmentation.homographic_augmentation(
                            optical,
                            my_keypoints[0],
                    ...
                    thermal, keypoints_thermal, valid_mask_thermal, H = (
                        augmentation.homographic_augmentation(
                            thermal,
                            my_keypoints[1],
                            return_homography=True,
                ...
                patch_size = self.config["augmentation"]["homographic"]["params"][
                    "corner_homography"
                ]["params"]["patch_size"]
                out["hm_input"], out["hfour_points"] = self.prep_hm_regression_input(
                    myopt,
                    myth,
                    optical_homography=out["optical"]["homography"],
                    thermal_homography=out["thermal"]["homography"],
                    top_left_point=[h // 2 - 64, w // 2 - 64],
                    patch_size_h_w=[128, 128],
                )
            else:
            ...
            out["optical"]["valid_mask"] = torch.from_numpy(
                valid_mask_optical.astype(bool)
            )
    ...
    def prep_hm_regression_input(
        ...
        for point in four_points:
            ...
            perturbed_four_points.append(
                [int(point_hom_transformed[0]), int(point_hom_transformed[1])]
            )

        H_four_points = np.subtract(
            np.array(perturbed_four_points), np.array(four_points)
        )  # / self.config["augmentation"]["homographic"]["params"]["corner_homography"]["params"]["rho"]
