...
from xpoint.datasets.augmentation import photometric_augmentation as photoaug
...
def homographic_augmentation(image, keypoints = None, return_homography=False, **config):
    ...
    if config["params"]["corner_homography"]["enable"]:
        homography = utils.sample_homography_corner(image_shape,config["params"]["corner_homography"]["params"]) #ADDED BY ME!
    else:
