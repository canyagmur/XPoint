...
def update_config(config,config_filename,config_dictionary=None): #, args):
    _update_config_from_file(config, config_filename)

    config.defrost()
    def merge_dicts(config, other_dict):
        for key, value in other_dict.items():
            if isinstance(value, dict):
                # If the value is a dictionary, recursively merge it
                merge_dicts(config.setdefault(key, {}), value)
            else:
                # If the value is not a dictionary, assign it to the corresponding key in config
                config[key] = value

    # Call the function to merge config_dictionary with config
    if config_dictionary:
        merge_dicts(config, config_dictionary)
    # print(config.MODEL.VSSM.SSM_FORWARDTYPE)
    # config.MODEL.VSSM.SSM_FORWARDTYPE = "v05_noz" #TODO LOOK FOR NOW V05_NOZ SET HERE !!!
...
def get_config(config_filename,config_dict=None): #args):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, config_filename,config_dict)
