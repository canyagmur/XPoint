...
from xpoint.models.RIFT2.src.RIFT2 import RIFT2
...
class ClassicDetectors(nn.Module):
    ...
    def __init__(self, config = None):
        ...
        elif self.config['method'] == "RIFT2":
            self.method = RIFT2()
            self.method_2 = RIFT2()
        elif self.config['method'] == "SRIF":
            self.method = SRIF()
            self.method_2 = SRIF()
            self.config['takes_pair'] = True
        else:
...
class LGHD():
    ...
    def detectAndCompute(self, image, mask):
        ...
        valid_keypoints = np.ones(len(keypoints)).astype(bool)
        descriptors = np.zeros((len(keypoints), 16 * self.n_scales * self.n_angles))
        for i, kp in enumerate(keypoints):
            pos =  np.array(kp.pt[::-1]).round().astype(int)
