...
def get_random_color(background_color, min_contrast = 0.13):
    """ Output a random scalar in grayscale with a least a small
        contrast with the background color """
    color = random.random()
    if abs(color - background_color) < min_contrast:  # not enough contrast
        color = (color + 0.5) % 1.0
    return color

def get_different_color(previous_colors, min_contrast = 0.13, max_count=20):
    ...
    color = random.random()
    count = 0
    while np.any(np.abs(previous_colors - color) < min_contrast) and count < max_count:
        count += 1
        color = random.random()
    return color
...
def generate_shape_background(size, background_color, min_contrast=0.13, nb_blobs=3000,
    ...
    img = img + get_random_color(background_color, min_contrast)
    ...
    for i in range(nb_blobs):
        col = get_random_color(background_color, min_contrast)
...
def generate_background(shape = [240, 320], nb_blobs=100, min_rad_ratio=0.01,
                        max_rad_ratio=0.05, min_kernel_size=50, max_kernel_size=300, min_contrast=0.13):
    ...
    dim = max(shape)
    cv2.threshold(img, random.random(), 1.0, cv2.THRESH_BINARY, img)
    background_color = np.mean(img)
    blobs = np.concatenate([np.random.randint(0, shape[1], size=(nb_blobs, 1)),
                            np.random.randint(0, shape[0], size=(nb_blobs, 1))],
                           axis=1)
    for i in range(nb_blobs):
        col = get_random_color(background_color, min_contrast)
        cv2.circle(img, (blobs[i][0], blobs[i][1]),
                   random.randint(int(dim * min_rad_ratio),
...
def draw_lines(img, nb_lines=10, min_contrast=0.13):
    ...
    for i in range(num_lines):
        ...
        col = get_random_color(np.mean(img), min_contrast)
        thickness = random.randint(int(math.ceil(min_dim * 0.01)), int(min_dim * 0.02))
        cv2.line(img, (x1, y1), (x2, y2), col, thickness)
        points = np.concatenate([points, np.array([[x1, y1], [x2, y2]])], axis=0)
    return points


def draw_polygon(img, max_sides=8, min_contrast=0.13):
    ...
    rad = max(random.random() * min_dim / 2, min_dim / 10)
    x = random.randint(int(rad), int(img.shape[1] - rad))  # Center of a circle
    y = random.randint(int(rad), int(img.shape[0] - rad))

    # Sample num_corners points inside the circle
    slices = np.linspace(0, 2 * math.pi, num_corners + 1)
    angles = [slices[i] + random.random() * (slices[i+1] - slices[i])
              for i in range(num_corners)]
    points = np.array([[int(x + max(random.random(), 0.4) * rad * math.cos(a)),
                        int(y + max(random.random(), 0.4) * rad * math.sin(a))]
                       for a in angles])
...
def draw_multiple_polygons(img, max_sides=8, nb_polygons=30, **extra):
    ...
    for i in range(nb_polygons):
        ...
        angles = [slices[i] + random.random() * (slices[i+1] - slices[i])
                  for i in range(num_corners)]
        new_points = [[int(x + max(random.random(), 0.4) * rad * math.cos(a)),
                       int(y + max(random.random(), 0.4) * rad * math.sin(a))]
                      for a in angles]
        ...
        custom_background = generate_shape_background(img.shape, background_color,
                                                       **extra)
        cv2.fillPoly(mask, [corners], 1.0)
...
def draw_ellipses(img, nb_ellipses=20, min_contrast=0.13):
    ...
    for i in range(nb_ellipses):
        ax = int(max(random.random() * min_dim, min_dim / 5))
        ay = int(max(random.random() * min_dim, min_dim / 5))
        max_rad = max(ax, ay)
        x = random.randint(max_rad, img.shape[1] - max_rad)  # center
        y = random.randint(max_rad, img.shape[0] - max_rad)
...
def draw_star(img, nb_branches=6, min_contrast=0.13):
    ...
    min_dim = min(img.shape[0], img.shape[1])
    thickness = random.randint(int(math.ceil(min_dim * 0.01)), int(min_dim * 0.02))
    rad = max(random.random() * min_dim / 2, min_dim / 5)
    ...
    points = np.array([[int(x + max(random.random(), 0.3) * rad * math.cos(a)),
                        int(y + max(random.random(), 0.3) * rad * math.sin(a))]
                       for a in angles])
...
def draw_checkerboard(img, max_rows=7, max_cols=7, transform_params=(0.05, 0.15), min_contrast=0.13):
    ...
    background_color = np.mean(img)
    # Create the grid
    rows = random.randint(3, max_rows)  # number of rows
    cols = random.randint(3, max_cols)  # number of cols
    ...
    alpha_affine = np.max(img.shape) * (transform_params[0]
                                        + random.random() * transform_params[1])
    center_square = np.float16(img.shape) // 2
    min_dim = min(img.shape)
    ...
    pts1 = np.float32([center_square + square_size,
                       ...
                       [center_square[0]-square_size, center_square[1]+square_size]])
    pts2 = pts1 + np.random.uniform(-alpha_affine,
                                    alpha_affine,
                                    size=pts1.shape).astype(np.float32)
    affine_transform = cv2.getAffineTransform(pts1[:3], pts2[:3])
    pts2 = pts1 + np.random.uniform(-alpha_affine / 2,
                                    alpha_affine / 2,
                                    size=pts1.shape).astype(np.float32)
    ...
    # Fill the rectangles
    colors = np.zeros((rows * cols,), np.float16)
    for i in range(rows):
        for j in range(cols):
            ...
            if i == 0 and j == 0:
                ...
                col = get_different_color(np.array(neighboring_colors), min_contrast)
            colors[i * cols + j] = col
            # Fill the cell
    ...
    nb_rows = random.randint(2, rows + 2)
    nb_cols = random.randint(2, cols + 2)
    thickness = random.randint(int(math.ceil(min_dim * 0.01)), int(min_dim * 0.015))
...
def draw_stripes(img, max_nb_cols=13, min_width_ratio=0.04,
                 transform_params=(0.05, 0.15), min_contrast=0.13):
    ...
    background_color = np.mean(img)
    # Create the grid
    board_size = (int(img.shape[0] * (1 + random.random())),
                  int(img.shape[1] * (1 + random.random())))
    col = random.randint(5, max_nb_cols)  # number of cols
    ...
    # Fill the rectangles
    color = get_random_color(background_color, min_contrast)
    for i in range(col):
        color = (color + 0.4 + random.random()*0.2) % 1.0
    ...
    nb_rows = random.randint(2, 5)
    nb_cols = random.randint(2, col + 2)
    thickness = random.randint(int(math.ceil(min_dim * 0.01)), int(min_dim * 0.015))
    for _ in range(nb_rows):
        row_idx = random.choice([0, col + 1])
...
def draw_cube(img, min_size_ratio=0.2, min_angle_rot=math.pi / 10,
              scale_interval=(0.4, 0.6), trans_interval=(0.5, 0.2), min_contrast=0.13):
    ...
    lx = min_side + random.random() * 2 * min_dim / 3  # dimensions of the cube
    ly = min_side + random.random() * 2 * min_dim / 3
    lz = min_side + random.random() * 2 * min_dim / 3
    cube = np.array([[0, 0, 0],
                     [lx, 0, 0],
                     [0, ly, 0],
                     [lx, ly, 0],
    ...
    scaling = np.array([[scale_interval[0] +
                         random.random() * scale_interval[1], 0, 0],
                        [0, scale_interval[0] +
                         random.random() * scale_interval[1], 0],
                        [0, 0, scale_interval[0] +
                         random.random() * scale_interval[1]]])
    trans = np.array([img.shape[1] * trans_interval[0] +
                      random.randint(-img.shape[1] * trans_interval[1],
                                           img.shape[1] * trans_interval[1]),
                      img.shape[0] * trans_interval[0] +
                      random.randint(-img.shape[0] * trans_interval[1],
    ...
    col_face = get_random_color(background_color, min_contrast)
    for i in [0, 1, 2]:
        cv2.fillPoly(img, [cube[faces[i]].reshape((-1, 1, 2))],
                    col_face)
    thickness = random.randint(int(math.ceil(min_dim * 0.003)), int(min_dim * 0.015))
    for i in [0, 1, 2]:
        for j in [0, 1, 2, 3]:
            col_edge = (col_face + 0.25
                        + random.random()*0.5)\
                        % 1.0  # color that constrats with the face color
...
def gaussian_noise(img, min_contrast=0.0):
    """ Apply random noise to the image """
    cv2.randu(img, 0.0, 1.0)
    return np.empty((0, 2), dtype=int)
