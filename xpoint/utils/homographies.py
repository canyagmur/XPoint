...
def homographic_adaptation_multispectral(data, net, homographic_adaptation_config={}):
    ...
    config = dict_update(homography_adaptation_default_config,
                         homographic_adaptation_config)

    if config['num'] < 1:
        raise ValueError(
            'num must be larger than 0 for the homographic adaptation')

    if config['filter_size'] % 2 == 0 and config['filter_size'] != 0:
        raise ValueError('The filter_size must be uneven')

    if config['aggregation'] == 'window' and config['filter_size'] > 0:
        raise ValueError(
            'Window aggregation assumes keypoints in the binary heatmap! (Filter size must be 0 if window is set)')

    image_shape = data['optical']['image'].shape

    # process the original images
    if net.takes_pair():
        out_optical,out_thermal,_ = net(data)
    else:
        out_optical = net(data['optical'])
        out_thermal = net(data['thermal'])
        
    #print(out_optical.keys(),out_optical["prob"].shape,out_optical["desc"].shape,out_optical["logits"].shape)
    #print(out_optical["logits"])
    #a = torch.nonzero(out_optical["prob"])
    # print(out_optical["prob"][0][0][11][605])
    # print("---")
    # print(out_optical["desc"][0][0][11][605])
    # print("---------")
    image_shape = out_optical["prob"].shape  # addded by me
    ...
    if config['aggregation'] == 'prod':
        ...
        prob = out_optical['prob'] + out_thermal['prob']
    elif config['aggregation'] == 'window':
        prob_optical, prob_thermal = search_window(
            out_optical['prob'], out_thermal['prob'], window_size=config["window_size"],weighted_window=config['weighted_window'])  # here
    else:
    ...
    for i in range(1, config['num']):
        ...
        homography = sample_homography(
            np.array(image_shape[2:]), **config['homographies'])
        valid_mask = compute_valid_mask(tuple(
            image_shape[2:]), homography, config['erosion_radius'], config['mask_border'])
        valid_mask = torch.from_numpy(valid_mask.astype(np.float32)).unsqueeze(
            0).unsqueeze(0).repeat(image_shape[0], 1, 1, 1).to(device)

        homography = torch.from_numpy(homography.astype(np.float32)).unsqueeze(
            0).repeat(image_shape[0], 1, 1).to(device)

        # predict the probabilites
        warped_images = warper(torch.cat([data['optical']['image'], data['thermal']['image']]), torch.cat(
            [homography, homography]), image_shape[2:], 'bilinear', 'reflection')
        ...
        if config['aggregation'] == 'prod':
            ...
            unwarped_optical = warper(out_optical['prob'], torch.inverse(
                homography), image_shape[2:], 'bilinear')
            unwarped_thermal = warper(out_thermal['prob'], torch.inverse(
                homography), image_shape[2:], 'bilinear')
            prob_unwarped_optical, prob_unwarped_thermal = search_window(
                unwarped_optical, unwarped_thermal, window_size=config["window_size"],weighted_window=config['weighted_window'])  # here
        else:
            raise ValueError('Unknown aggregation: ' + config['aggregation'])

        count_sample = warper(valid_mask, torch.inverse(
            homography), image_shape[2:], 'nearest')
        count += count_sample

        if config['aggregation'] == 'window':
            prob_optical += prob_unwarped_optical*count_sample
            prob_thermal += prob_unwarped_thermal*count_sample

        else:
            prob += warper(prob_warped, torch.inverse(homography),
                           image_shape[2:], 'bilinear') * count_sample

    if config['aggregation'] == 'window':
        out_prob_optical = prob_optical / count
        out_prob_thermal = prob_thermal / count
    ...
    if config['min_count'] > 0 and config['aggregation'] != 'window':
        out[count < config['min_count']] = 0.0  # look here as well
    elif config['min_count'] > 0 and config['aggregation'] == 'window':
        out_prob_optical[count < config['min_count']] = 0.0  # look here as well
        out_prob_thermal[count < config['min_count']] = 0.0  # look here as well

    if config['aggregation'] != 'window':
        out_prob_optical = None
        out_prob_thermal = None
    else:
        out = None

    out_dict =  {"out": {},"out_optical":{},"out_thermal":{}}
    
    out_dict["out_optical"]["prob"] = out_prob_optical
    out_dict["out_thermal"]["prob"] = out_prob_thermal
    out_dict["out"]["prob"] = out


    out_dict["desc_optical"] = out_optical["desc"]
    out_dict["desc_thermal"] = out_thermal["desc"]

    return out_dict #out, out_optical, out_thermal


def search_window(tensor1, tensor2, window_size,weighted_window):
    assert window_size % 2 != 0

    def transform_pad(tens, window_size):
        pad = torch.nn.ZeroPad2d(int((window_size - 1) / 2))
        padded_tens = pad(tens)
        return padded_tens

    def transform_unfolded(tens, window_size):  # it pads and unfolds
        unfolded = tens.unfold(2, window_size, 1).unfold(
            3, window_size, 1)  # Tensor.unfold(dimension, size, step)
        return unfolded.clone()

    padded_x1 = transform_pad(tensor1, window_size)
    padded_x2 = transform_pad(tensor2, window_size)

    unfolded_x1 = transform_unfolded(padded_x1, window_size)
    unfolded_x2 = transform_unfolded(padded_x2, window_size)

    filtered_x1 = torch.sum(input=unfolded_x2, dim=[4, 5])*tensor1
    filtered_x2 = torch.sum(input=unfolded_x1, dim=[4, 5])*tensor2

    if not weighted_window:
        filtered_x1[filtered_x1 > 0] = 1 # HERE IMPORTANT for  not weighted keypoint
        filtered_x2[filtered_x2 > 0] = 1 #
    
    return filtered_x1, filtered_x2
...
def homographic_adaptation(data, net, homographic_adaptation_config={}):
    ...
    for i in range(1, config['num']):
        ...
        # predict the probabilites
        warped_images = warper(
            data['image'], homography, image_shape[2:], 'bilinear', 'reflection')
        input = {
            'image': warped_images,
        }
        out = net(input)
        ...
        count_sample = warper(valid_mask, torch.inverse(
            homography), image_shape[2:], 'nearest')
        count += count_sample
        prob += warper(out['prob'], torch.inverse(homography),
                       image_shape[2:], 'bilinear') * count_sample
...
def sample_homography(image_shape, perspective=True, scaling=True, rotation=True, translation=True,
                      ...
                      allow_artifacts=True, translation_overflow=0.1,corner_homography={}):
    """
    ...
    def transform_perspective(points):
        ...
        if not allow_artifacts:
            perspective_amplitude_min = np.maximum(
                np.array([-perspective_amplitude_x, -perspective_amplitude_y]), t_min)
            perspective_amplitude_max = np.minimum(
                np.array([perspective_amplitude_x, perspective_amplitude_y]), t_max)
        else:
            perspective_amplitude_min = np.array(
                [-perspective_amplitude_x, -perspective_amplitude_y])
            perspective_amplitude_max = np.array(
                [perspective_amplitude_x, perspective_amplitude_y])

        perspective_displacement = np.random.uniform(
            perspective_amplitude_min[1], perspective_amplitude_max[1])
        h_displacement_left = np.random.uniform(
            perspective_amplitude_min[0], perspective_amplitude_max[0])
        h_displacement_right = np.random.uniform(
            perspective_amplitude_min[0], perspective_amplitude_max[0])
    ...
    def transform_scale(points):
        scales = np.random.uniform(-scaling_amplitude,
                                   scaling_amplitude, n_scales) + 1.0
        center = points.mean(axis=0)
        scaled = np.expand_dims(
            points - center, 0) * np.expand_dims(np.expand_dims(scales, 1), 1) + center
    ...
    def transform_rotation(points):
        ...
        center = points.mean(axis=0)
        rot_mat = np.reshape(np.stack([np.cos(
            angles), -np.sin(angles), np.sin(angles), np.cos(angles)], axis=1), [-1, 2, 2])
        rotated = np.matmul(np.tile(np.expand_dims(
            points - center, axis=0), [n_angles+1, 1, 1]), rot_mat) + center
        if allow_artifacts:
...
def sample_homography_corner(image_shape,config):
    import random
    imsize = image_shape[1],image_shape[0]
    rho = config["rho"]
    patch_size = config["patch_size"]
    # Get random patch positions
    ...
    # Perturb the four patch corners
...
def warp_keypoints(keypoints, homography, return_type=int):
    ...
    if len(keypoints) > 0:
        warped_points = cv2.perspectiveTransform(
            np.array([keypoints[:, ::-1]], dtype=np.float64), homography)
...
def warp_points_pytorch(points, homography):
    ...
    warped_points = torch.cat([points.flip(-1), torch.ones([points.shape[0],
                              points.shape[1], 1], dtype=torch.float32, device=points.device)], -1)

    # apply homography
    warped_points = torch.bmm(
        homography, warped_points.permute([0, 2, 1])).permute([0, 2, 1])
...
def compute_valid_mask(image_shape, homography, erosion_radius=0, mask_border=False):
    ...
    mask = cv2.warpPerspective(
        np.ones(image_shape), homography, image_shape[::-1], flags=cv2.INTER_NEAREST)
    ...
    if erosion_radius > 0:
        if mask_border:
            ...
            mask = tmp
        kernel = np.ones(
            (erosion_radius * 2 + 1, erosion_radius * 2 + 1), np.float32)
...
def warp_perspective_tensor(src, M, dsize, mode='bilinear', padding_mode='zeros'):
    ...
    if not kornia_available:
        raise RuntimeError(
            'Kornia not imported but required for warp_perspective_tensor')
    ...
    if not (len(M.shape) == 3 or M.shape[-2:] == (3, 3)):
        raise ValueError("Input M must be a Bx3x3 tensor. Got {}"
                         .format(src.shape))
    # launches the warper
    M_norm = kornia.geometry.transform.imgwarp.dst_norm_to_dst_norm(
        M, (src.shape[-2:]), dsize)
